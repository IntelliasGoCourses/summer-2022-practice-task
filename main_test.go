...
func TestTrains(t *testing.T) {
	...
	for _, elem := range caseMap {
		res, _ := FindTrains(elem.depStation, elem.arrStation, elem.criteria)
		if !reflect.DeepEqual(res, elem.exp) {
			t.Errorf("Wrong result of %v + %v + %v: got %v, expected %v or %v.", elem.depStation, elem.arrStation, elem.criteria, res, elem.exp, elem.expErr)
		}
	}
}
